...
fn test_lora_layer_gradient_flow() {
    ...
    let grad_a_sum = grad_a.abs().unwrap().sum_all().unwrap().to_vec0::<f32>().unwrap();
    let grad_b_sum = grad_b.abs().unwrap().sum_all().unwrap().to_vec0::<f32>().unwrap();

    assert!(grad_a_sum > 1e-6, "Gradient A should be non-zero, got {}", grad_a_sum);
    assert!(grad_b_sum > 1e-6, "Gradient B should be non-zero, got {}", grad_b_sum);
}
...
fn test_lora_adapter_gradient_collection() {
    ...
    let lora_adapter =
        LoRAAdapter::new(32, 128, 2, &adapter_config, &device).unwrap();
    ...
    // Apply LoRA delta
    let delta = lora_adapter.forward(0, &TargetModule::QProj, &input).unwrap();
...
fn test_zero_gradients_with_frozen_params() {
    ...
    assert!(grads.get(lora.lora_a()).is_some(), "LoRA A should have gradients");
    assert!(grads.get(lora.lora_b()).is_some(), "LoRA B should have gradients");
