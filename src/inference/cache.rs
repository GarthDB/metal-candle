...
impl KVCache {
    ...
    pub fn update(
        ...
        let (full_key, full_value) = if let Some((cached_key, cached_value)) = self.cache.get(&layer_idx) {
        ...
        // Store updated cache
        self.cache.insert(layer_idx, (full_key.clone(), full_value.clone()));
...
mod tests {
    ...
    fn test_kv_cache_full() {
        ...
        cache.update(0, key.clone(), value.clone()).unwrap();
        cache.update(0, key.clone(), value.clone()).unwrap();
